...
const BlogListItem = ({post, updateViewingPost, getPosts}) => {
    ...
    async function handleUpdatePublished() {
        ...
        if (usernameData) {
            ...
            fetch(`https://blog-api-production-346d.up.railway.app/users/${parsedUsernameData.username}/posts/${post.id}`, {
                mode: 'cors',
                method: 'PUT',
                headers: {
    ...
    async function deletePost(postId) {
        ...
        if (usernameData) {
          const parsedUsernameData = JSON.parse(usernameData);
          fetch(`https://blog-api-production-346d.up.railway.app/users/${parsedUsernameData.username}/posts/${postId}`, {
            mode: 'cors',
            method: 'DELETE',
            headers: {
              "Authorization": `Bearer ${token}`
